...
static unsigned short readReg16(unsigned char ucAddr) {
    ...
    i2c_write(TOF_ADDR,(char *)&ucAddr, 1);
    i2c_read(TOF_ADDR, (char *)ucTemp, 2);
...
static unsigned char readReg(unsigned char ucAddr) {
    ...
    i2c_write(TOF_ADDR, (char *)&ucTemp, 1);
    i2c_read(TOF_ADDR, (char *)&ucTemp,1);
    return ucTemp;
} /* ReadReg() */

static void readMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount) {
    i2c_write(TOF_ADDR,(char *)&ucAddr, 1);
    i2c_read(TOF_ADDR, (char *)pBuf, iCount);
} /* readMulti() */
...
static void writeMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount) {
    ...
    ucTemp[0] = ucAddr;
    memcpy(&ucTemp[1], (char *)pBuf, iCount);
    i2c_write(TOF_ADDR,(char *)ucTemp, iCount+1);
} /* writeMulti() */
...
static void writeReg16(unsigned char ucAddr, unsigned short usValue) {
    ...
    i2c_write(TOF_ADDR, (char *)ucTemp, 3);
...
static void writeReg(unsigned char ucAddr, unsigned char ucValue) {
    ...
    ucTemp[0] = ucAddr;
    ucTemp[1] = ucValue;
    i2c_write(TOF_ADDR, (char *)ucTemp, 2);
} /* writeReg() */
...
static void writeRegList(unsigned char *ucList){
    ...
    while (ucCount){
        i2c_write(TOF_ADDR, (char *)ucList, 2);
        ucList += 2;
        ucCount--;
    }
} /* writeRegList() */
